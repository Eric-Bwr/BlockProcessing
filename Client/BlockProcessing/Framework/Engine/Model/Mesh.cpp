...
Mesh::Mesh(const float *vertices, uint64_t verticesSize, bool skeletal, unsigned int drawType, unsigned int drawMode) : drawType(drawType) {
    vao.init();
    if(!skeletal) {
...
Mesh::Mesh(const float *vertices, uint64_t verticesSize, const unsigned int *indices, uint64_t indicesSize, bool skeletal, unsigned int drawType, unsigned int drawMode) : numIndices(indicesSize), drawType(drawType) {
    vao.init();
    if(!skeletal) {
        numVertices = verticesSize / 14;
        vbo.init(vertices, ModelLoader::layout.getStride() * numVertices, drawMode);
