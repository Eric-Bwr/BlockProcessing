...
void Model::load() {
    ...
    for (auto &meshData: resourceSceneData.meshes) {
        ...
        auto mesh = new Mesh(data.data(), data.size(), meshData.indices.data(), meshData.indices.size(), false, drawType, drawMode);
...
void Model::loadSkeletal() {
    ...
    for (auto &meshData: resourceSceneData.meshes) {
        ...
        auto mesh = new Mesh(data.data(), data.size(), meshData.indices.data(), meshData.indices.size(), true, drawType, drawMode);
        mesh->setMaterial(materials[meshData.materialIndex]);
        meshes.emplace_back(mesh);
    }
}
