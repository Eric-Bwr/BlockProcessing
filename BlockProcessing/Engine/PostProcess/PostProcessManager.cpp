...
PostProcessManager::PostProcessManager(int width, int height) : width(width), height(height) {
    ...
    shader = new Shader(postProcessShader);
    blurHorizontalShader = new Shader(postBlurHShader);
    blurVerticalShader = new Shader(postBlurVShader);
    float vertices[8] = {-1, 1, -1, -1, 1, 1, 1, -1};
    screenModel = new Model2D(vertices, 8, GL_TRIANGLE_STRIP);
    frameBuffer->unbind();
}
