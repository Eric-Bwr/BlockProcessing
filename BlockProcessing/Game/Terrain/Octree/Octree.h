...
public:
    ...
    int current_nodes_index, int& chunk_index);
    ~OctreeNode();
...
class Octree{
public:
    explicit Octree(const Coord& coord) :
    ...
    chunks(1 << (3*OCTREE_MAX_LEVEL)) {
        ...
        int chunk_index = 0;
        auto node = OctreeNode(OCTREE_MAX_LEVEL, OCTREE_LENGTH, coord, nodes, chunks, 0, chunk_index);//build the root node
